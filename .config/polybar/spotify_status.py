...
# Default parameters
output = fix_string(u'{play_pause} {artist} [{album} | {track_number}] {song}')
trunclen = 65
...
try:
    ...
    if (track_number == 0 or song == 'Advertisement'):
       rc = subprocess.call("skipspotifyad")

    if (quiet and status == 'Paused') or (not artist and not song and not album):
        rc = subprocess.call("./check_spotify_running.sh")
        if(rc == 1):
            # spotify is running, but it's a sneaky advertisement
           rc = subprocess.call("skipspotifyad")
        else:
            print('no jams ._.' + str(track_number) + " <- tn")
    else:
        if font:
